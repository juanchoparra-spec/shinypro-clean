...
export default function App() {
  ...
  const sincronizarCalendar = async (action, trabajoCompleto, googleEventId) => {
    try {
      ...
      const payload = {
        ...
        trabajo: {
          fecha: trabajoCompleto.fecha,
          hora: trabajoCompleto.hora,
          duracion: trabajoCompleto.duracion,
          valor: trabajoCompleto.valor,
          pago: trabajoCompleto.pago,
          estado: trabajoCompleto.estado,
          agendado_por: trabajoCompleto.agendadoPor,
          notas: trabajoCompleto.notas,
      ...
      if (error) { console.error("Error sincronizando calendario:", error); return null; }
      ...
      try {
        ...
        const nuevos = rows.map(r => {
          ...
          return {
            ...
            tipo_cliente: get("tipo_cliente", "tipo de cliente", "tipo", "type"),
  ...
  /* genera lista de fechas según patrón de repetición (incluye la fecha inicial) */
  const generarFechasRepeticion = (fechaInicial, repetir) => {
    const fechas = [fechaInicial];
    if (repetir === "ninguna") return fechas;
    if (repetir === "2semanas") {
      fechas.push(sumarDias(fechaInicial, 14));
    } else if (repetir === "15dias") {
      let f = fechaInicial;
      for (let i = 0; i < 23; i++) { // ~1 año cada 15 días
        f = sumarDias(f, 15);
        fechas.push(f);
      }
    } else if (repetir === "mensual") {
      for (let i = 1; i <= 11; i++) {
        fechas.push(sumarMeses(fechaInicial, i));
      }
    } else if (repetir === "anual_semanal") {
      // todo el año, misma frecuencia semanal (cada 7 dias) por 52 semanas
      for (let i = 1; i <= 51; i++) {
        fechas.push(sumarDias(fechaInicial, i * 7));
      }
    }
    return fechas;
  };
  ...
  const guardarTrabajo = async (data) => {
    ...
    if (id) {
      ...
      if (original?.serieId && aplicarA === "futuros") {
        ...
        for (const t of futuros) {
          ...
          const { data: actualizado } = await supabase.from("trabajos").update(payloadT).eq("id", t.id).select().single();
          if (actualizado) {
            const tf = trabajoDesdeDb(actualizado);
            const action = tf.google_event_id ? "update" : "create";
            const googleEventId = await sincronizarCalendar(action, tf, tf.google_event_id);
            if (googleEventId && googleEventId !== tf.google_event_id) {
              await supabase.from("trabajos").update({ google_event_id: googleEventId }).eq("id", tf.id);
              tf.google_event_id = googleEventId;
            }
            setTrabajos(prev => prev.map(p => p.id === tf.id ? tf : p));
          }
        }
      } else {
        const { data: actualizado } = await supabase.from("trabajos").update(payload).eq("id", id).select().single();
        if (actualizado) {
          const trabajoFinal = trabajoDesdeDb(actualizado);
          const action = trabajoFinal.google_event_id ? "update" : "create";
          const googleEventId = await sincronizarCalendar(action, trabajoFinal, trabajoFinal.google_event_id);
          if (googleEventId && googleEventId !== trabajoFinal.google_event_id) {
            await supabase.from("trabajos").update({ google_event_id: googleEventId }).eq("id", trabajoFinal.id);
            trabajoFinal.google_event_id = googleEventId;
          }
          setTrabajos(prev => prev.map(p => p.id === id ? trabajoFinal : p));
        }
      }
      setModal(null);
      return;
    ...
    for (const fecha of fechas) {
      ...
      if (creado) {
        ...
        const googleEventId = await sincronizarCalendar("create", tf, null);
        if (googleEventId) {
          await supabase.from("trabajos").update({ google_event_id: googleEventId }).eq("id", tf.id);
          tf.google_event_id = googleEventId;
        }
        nuevosTrabajos.push(tf);
      }
    }
    setTrabajos(prev => [...prev, ...nuevosTrabajos]);
    setModal(null);
  };

  const borrarTrabajo = async (id, aplicarA) => {
    const trabajo = trabajos.find(t => t.id === id);
    if (trabajo?.serieId && aplicarA === "futuros") {
      const futuros = trabajos.filter(t => t.serieId === trabajo.serieId && t.fecha >= trabajo.fecha);
      for (const t of futuros) {
        if (t.google_event_id) sincronizarCalendar("delete", t, t.google_event_id);
  ...
  const borrarAgendador = async (nombre) => {
    if (!window.confirm(`¿Eliminar "${nombre}" de la lista de quién agenda?`)) return;
    await supabase.from("agendadores").delete().eq("nombre", nombre);
  ...
  return (
    <div className="sp-root" style={{ paddingBottom: 90 }}>
      ...
      <main style={{ maxWidth: 640, margin: "0 auto", padding: "0 16px" }}>
        {vista === "agenda" && (
          <VistaAgenda
            ...
            onBorrar={(t) => {
              if (t.serieId) {
                setModal({ tipo: "confirmarBorrado", data: t });
              } else {
                if (window.confirm("¿Eliminar este trabajo?")) borrarTrabajo(t.id);
              }
            }}
        ...
        {vista === "clientes" && (
          <VistaClientes
            ...
            onAgendar={(c) => setModal({ tipo: "trabajo", clientePreseleccionado: c })}
      ...
      {modal?.tipo === "trabajo" && (
        <ModalTrabajo
          data={modal.data}
          clientePreseleccionado={modal.clientePreseleccionado}
          clientes={clientes}
          agendadores={agendadores}
          onAgregarAgendador={agregarAgendador}
          onBorrarAgendador={borrarAgendador}
      ...
      {modal?.tipo === "confirmarBorrado" && (
        <ModalConfirmarSerie
          titulo="Eliminar trabajo recurrente"
          mensaje="Este trabajo es parte de una serie repetida. ¿Qué deseas eliminar?"
          onSoloEste={() => { borrarTrabajo(modal.data.id, "solo"); setModal(null); }}
          onFuturos={() => { borrarTrabajo(modal.data.id, "futuros"); setModal(null); }}
          onCerrar={() => setModal(null)}
        />
      )}
    </div>
          ...
          <div style={{ display: "flex", flexDirection: "column", gap: 10 }}>
            {grupos[fecha].map(t => (
              <TrabajoCard key={t.id} t={t} cliente={clientesPorId[t.clienteId]}
                onEditar={() => onEditar(t)} onBorrar={() => onBorrar(t)} onToggle={() => onToggle(t.id)} />
            ))}
          </div>
        </div>
      ))}
  ...
  return (
    <div className="sp-card" style={{ padding: 14, borderLeft: `4px solid ${completado ? "var(--mint)" : "var(--coral)"}` }}>
      ...
      <div style={{ display: "flex", flexWrap: "wrap", gap: 8, marginTop: 10, alignItems: "center" }}>
        ...
        {t.serieId && (
          <div className="sp-tag" style={{ background: "#FFF3D6", color: "#A6791E" }}>
            🔁 Recurrente
          </div>
        )}
      </div>
        ...
        <div key={c.id} className="sp-card" style={{ padding: 14 }}>
          <div style={{ display: "flex", justifyContent: "space-between", alignItems: "flex-start" }}>
            <div>
              ...
              {c.tipo_cliente && <span className="sp-tag" style={{ background: "var(--mint-light)", color: "var(--teal)", marginTop: 4 }}>{c.tipo_cliente}</span>}
            </div>
          </div>
          {c.telefono && (
          ...
          <div style={{ display: "flex", gap: 8, marginTop: 12, borderTop: "1px solid #F1EBE2", paddingTop: 10 }}>
            <button onClick={() => onAgendar(c)} className="sp-btn sp-btn-primary" style={{ padding: "7px 14px", fontSize: 13, display: "flex", alignItems: "center", gap: 4 }}>
              <Calendar size={14} /> Agendar
            </button>
            <button onClick={() => onEditar(c)} className="sp-btn" style={{ background: "var(--mint-light)", color: "var(--teal)", padding: "7px 14px", fontSize: 13, display: "flex", alignItems: "center", gap: 4 }}>
              <Edit2 size={14} /> Editar
            </button>
            <button onClick={() => onBorrar(c.id)} className="sp-btn" style={{ background: "#FFE9E5", color: "var(--coral)", padding: "7px 14px", fontSize: 13, display: "flex", alignItems: "center", gap: 4 }}>
              <Trash2 size={14} /> Eliminar
            </button>
          </div>
        </div>
...
const VistaReportes = ({ trabajos, clientesPorId }) => {
  ...
  const porTipo = useMemo(() => {
    const acc = {};
    filtrados.forEach(t => {
      const tipo = clientesPorId[t.clienteId]?.tipo_cliente?.trim() || "Sin tipo";
      if (!acc[tipo]) acc[tipo] = { total: 0, cantidad: 0 };
      acc[tipo].total += Number(t.valor || 0);
      acc[tipo].cantidad += 1;
    });
    return acc;
  }, [filtrados, clientesPorId]);
    ...
    <div style={{ paddingTop: 16 }}>
      ...
      <div className="sp-card" style={{ padding: 16, marginBottom: 16 }}>
        ...
        {Object.keys(porTipo).length > 0 && (
          <div style={{ marginTop: 14, paddingTop: 12, borderTop: "1px solid #F1EBE2" }}>
            <div className="sp-label" style={{ marginBottom: 6 }}>Por tipo de cliente</div>
            <div style={{ display: "flex", flexDirection: "column", gap: 4, fontSize: 13 }}>
              {Object.entries(porTipo).map(([tipo, info]) => (
                <div key={tipo} style={{ display: "flex", justifyContent: "space-between" }}>
                  <span style={{ color: "#7A8B85" }}>{tipo} ({info.cantidad})</span>
                  <span style={{ fontWeight: 600 }}>${info.total.toFixed(2)}</span>
                </div>
              ))}
            </div>
          </div>
    ...
    <div className="sp-modalbg" onClick={onCerrar}>
      <div className="sp-modal" onClick={e => e.stopPropagation()}>
        ...
        <div style={{ display: "flex", flexDirection: "column", gap: 14, marginTop: 16 }}>
          ...
          <div>
            <span className="sp-label">Tipo de cliente</span>
...
const ModalTrabajo = ({ data, clientePreseleccionado, clientes, agendadores, onAgregarAgendador, onBorrarAgendador, onGuardar, onCrearClienteRapido, onCerrar }) => {
  ...
  const [form, setForm] = useState(data || {
    clienteId: clienteInicial?.id || "", fecha: todayISO(), hora: "09:00", duracion: 4,
  ...
  const [repetir, setRepetir] = useState("ninguna");
  const [mostrarConfirmSerie, setMostrarConfirmSerie] = useState(false);
  const [guardando, setGuardando] = useState(false);

  const clienteSeleccionado = clientes.find(c => c.id === form.clienteId);

  const sugerencias = useMemo(() => {
    if (!busquedaCliente || busquedaCliente === clienteSeleccionado?.nombre) return [];
    return clientes.filter(c => c.nombre.toLowerCase().includes(busquedaCliente.toLowerCase())).slice(0, 6);
  }, [busquedaCliente, clientes, clienteSeleccionado]);

  const guardarConOpcion = async (aplicarA) => {
    ...
    <div className="sp-modalbg" onClick={onCerrar}>
        ...
        <div style={{ display: "flex", flexDirection: "column", gap: 14, marginTop: 16 }}>
            ...
            {!creandoCliente ? (
              <div style={{ position: "relative" }}>
                ...
                {mostrarSugerencias && sugerencias.length > 0 && (
                  <div style={{ position: "absolute", top: "100%", left: 0, right: 0, background: "#fff", border: "1.5px solid var(--mint)", borderRadius: 10, zIndex: 100, boxShadow: "0 4px 12px rgba(0,0,0,0.1)", maxHeight: 200, overflowY: "auto" }}>
                    {sugerencias.map(c => (
                      <div key={c.id} onClick={() => { setForm({ ...form, clienteId: c.id }); setBusquedaCliente(c.nombre); setMostrarSugerencias(false); }}
                        style={{ padding: "10px 14px", cursor: "pointer", borderBottom: "1px solid #F1EBE2", fontSize: 14 }}
                        onMouseEnter={e => e.currentTarget.style.background = "var(--mint-light)"}
                        onMouseLeave={e => e.currentTarget.style.background = "transparent"}>
                        <div style={{ fontWeight: 600 }}>{c.nombre}</div>
                        {c.tipo_cliente && <div style={{ fontSize: 12, color: "#7A8B85" }}>{c.tipo_cliente}</div>}
                      </div>
                    ))}
                  </div>
                )}
          ...
          <div>
            ...
            {agendadores.length > 0 && (
              <div style={{ display: "flex", flexWrap: "wrap", gap: 6, marginTop: 8 }}>
                {agendadores.map(a => (
                  <span key={a} className="sp-tag" style={{ background: "#F4EFE6", color: "var(--charcoal)" }}>
                    {a}
                    <button onClick={() => onBorrarAgendador(a)} style={{ background: "none", border: "none", cursor: "pointer", padding: 0, marginLeft: 4, color: "var(--coral)", display: "flex" }} aria-label={`Eliminar ${a}`}>
                      <X size={12} />
                    </button>
                  </span>
                ))}
          ...
          {!data && (
            <div>
              ...
              <select className="sp-input" value={repetir} onChange={e => setRepetir(e.target.value)}>
                <option value="ninguna">No repetir</option>
                <option value="2semanas">Cada 2 semanas (esta y la siguiente)</option>
                <option value="15dias">Cada 15 días (por ~1 año)</option>
                <option value="mensual">Cada mes (por 1 año)</option>
                <option value="anual_semanal">Toda la semana, todo el año</option>
              </select>
              {repetir !== "ninguna" && (
                <div style={{ fontSize: 12, color: "#A6B5AF", marginTop: 4 }}>
                  Se crearán varios trabajos repetidos a partir de esta fecha, cada uno sincronizado con el calendario.
                </div>
              )}
            </div>
          )}
      ...
      {mostrarConfirmSerie && !guardando && (
        <ModalConfirmarSerie
          ...
          mensaje="Este trabajo es parte de una serie repetida. ¿A cuáles aplicar este cambio?"
          onSoloEste={() => { setMostrarConfirmSerie(false); guardarConOpcion("solo"); }}
          onFuturos={() => { setMostrarConfirmSerie(false); guardarConOpcion("futuros"); }}
          onCerrar={() => setMostrarConfirmSerie(false)}
        />
      )}
    </div>
  );
