...
export default function App() {
  ...
  const guardarTrabajo = async (data) => {
    ...
    if (id) {
      ...
      if (actualizado) trabajoFinal = trabajoDesdeDb(actualizado);
    } else {
      const { data: creado } = await supabase.from("trabajos").insert(payload).select().single();
      if (creado) trabajoFinal = trabajoDesdeDb(creado);
    }
    if (trabajoFinal) {
      ...
      if (id) setTrabajos(prev => prev.map(t => t.id === id ? trabajoFinal : t));
      else setTrabajos(prev => [...prev, trabajoFinal]);
    }
    setModal(null);
  };
    ...
    <div className="sp-modalbg" onClick={onCerrar}>
      <div className="sp-modal" onClick={e => e.stopPropagation()}>
        ...
        <div style={{ display: "flex", flexDirection: "column", gap: 14, marginTop: 16 }}>
          ...
          <div><span className="sp-label">Tipo de cliente</span>
            <input className="sp-input" value={form.tipo_cliente || ""} onChange={e => setForm({ ...form, tipo_cliente: e.target.value })} placeholder="Ej: Residencial, Comercial, Airbnb..." /></div>
